...
export class World {
  ...
  private generateTerrainUntil(worldX: number): void {
    while (this.terrainCursorX < worldX) {
      ...
      let y = makeValley ? this.rng.range(mid + 35, TERRAIN_MAX_Y) : this.rng.range(TERRAIN_MIN_Y, mid - 25)
      y += Math.sin(x / 1160) * 30 + Math.sin(x / 520 + 1.8) * 20
      if (Math.abs(y - this.terrainLastY) < 55) y += (y >= this.terrainLastY ? 1 : -1) * this.rng.range(55, 100)
