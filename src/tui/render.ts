...
export function renderGame(buffer: OptimizedBuffer, game: RopeManGame, options: RenderOptions): void {
  const width = buffer.width
  const height = buffer.height
  buffer.clear(COLORS.paper)

  if (width < 80 || height < 24) {
    drawCentered(buffer, "Rope Man needs at least 80x24 terminal cells", Math.floor(height / 2), COLORS.spike)
  ...
  if (snapshot.screen === "paused") drawOverlay(buffer, "PAUSED", snapshot.message, "Esc resume · R retry · H menu · Q quit")
...
function drawHud(buffer: OptimizedBuffer, snapshot: GameSnapshot, width: number): void {
  ...
  if (width > right.length + 4) buffer.drawText(right, width - right.length - 1, 0, COLORS.muted, COLORS.panel)
}
...
function drawMenu(buffer: OptimizedBuffer, snapshot: GameSnapshot, options: RenderOptions): void {
  ...
  drawCentered(buffer, `seed: ${draft}`, 15, options.seedError ? COLORS.spike : COLORS.focus, TextAttributes.BOLD)
...
function drawWorld(buffer: OptimizedBuffer, snapshot: GameSnapshot): void {
  ...
  for (const obstacle of snapshot.obstacles) drawObstacle(buffer, obstacle, snapshot.time, toCell, scaleX, scaleY)
...
function drawObstacle(
  ...
  if (obstacle.type === "saw") {
    const c = toCell({ x: obstacle.x, y: obstacle.y + Math.sin(time * 1.5 + obstacle.phase) * obstacle.bob * 0.25 })
  ...
  const y = obstacle.ground ? toCell({ x: obstacle.x, y: 610 }).y : toCell({ x: obstacle.x, y: obstacle.y }).y
...
function drawAnchor(buffer: OptimizedBuffer, anchor: Anchor, focused: Anchor | null, toCell: (p: Vec2) => { x: number; y: number }): void {
...
function drawRope(buffer: OptimizedBuffer, player: Player, toCell: (p: Vec2) => { x: number; y: number }): void {
...
function drawPlayer(buffer: OptimizedBuffer, player: Player, toCell: (p: Vec2) => { x: number; y: number }): void {
...
function drawOverlay(buffer: OptimizedBuffer, title: string, line1: string, line2: string): void {
  ...
  for (let yy = 0; yy < 7; yy++) {
    for (let xx = 0; xx < boxW; xx++) buffer.drawText(" ", x + xx, y + yy, COLORS.ink, COLORS.panel)
  }
...
function drawTerrain(buffer: OptimizedBuffer, points: { x: number; y: number }[], top: number): void {
...
function drawLine(buffer: OptimizedBuffer, x0: number, y0: number, x1: number, y1: number, color: RGBA): void {
...
function drawCentered(buffer: OptimizedBuffer, text: string, y: number, fg: RGBA, attributes = 0): void {
  const x = Math.max(0, Math.floor((buffer.width - text.length) / 2))
  drawTextAt(buffer, text, x, y, fg, undefined, attributes)
}

function drawTextAt(buffer: OptimizedBuffer, text: string, x: number, y: number, fg: RGBA, bg?: RGBA, attributes = 0): void {
  if (y < 0 || y >= buffer.height || x >= buffer.width) return
  buffer.drawText(text.slice(0, Math.max(0, buffer.width - x)), Math.max(0, x), y, fg, bg, attributes)
}
