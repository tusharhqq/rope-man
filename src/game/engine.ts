...
export class RopeManGame {
  ...
  update(dt: number): void {
    ...
    if (this.ropeShot && this.ropeShot.t >= this.ropeShot.duration) {
      ...
      if (
        ...
        shot.anchor &&
        Math.hypot(shot.anchor.x - this.player.x, shot.anchor.y - this.player.y) <=
          HOOK_RANGE + ROPE_ATTACH_GRACE
      ) {
  ...
  snapshot(): GameSnapshot {
    ...
    return {
      ...
      ropeShot: this.ropeShot
        ? {
            anchor: { ...this.ropeShot.anchor },
            t: this.ropeShot.t,
            duration: this.ropeShot.duration,
          }
        : null,
  ...
  private updateAttached(dt: number): void {
    ...
    this.player.angularVelocity =
      (this.player.vx * ny - this.player.vy * nx) / Math.max(1, this.player.ropeLength);
  }
  ...
  private findFocusedAnchor(): Anchor | null {
    ...
    for (const anchor of this.world.anchors) {
      ...
      const alignment = directionBias
        ? (dx / Math.max(1, d)) * aimX + (dy / Math.max(1, d)) * aimY
        : 0;
...
export function hitsObstacle(
  ...
  if (!inX) return false;
  return obstacle.ground
    ? player.y + 18 > spikeY - obstacle.size * 1.35
    : player.y - 18 < spikeY + obstacle.size * 1.35;
}
